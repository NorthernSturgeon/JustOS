...
//#define SHT_INITIAL_SIZE 4096

//uint64_t *st_ptr;
//size_t sht_size = SHT_INITIAL_SIZE;

void* memmove_temp(void* dst, void* src, size_t cnt){
	void* ret = dst;

	if (src+cnt > dst){
		dst += cnt;
		src += cnt;
		for (size_t i=0; i < cnt; i++) *((char*)--dst) = *((char*)--src);
	} else {
		for (size_t i=0; i < cnt; i++) *((char*)dst++) = *((char*)src++);
	}
		...
		for (size_t i = 0; i < gorl.lenght; i++){
			if (gorl.list[i] >= start){
				if (i&1){ // i odd [A(i-1)<start, Bi >= start]
					memmove_temp(&gorl.list[i+1], &gorl.list[i-1], (gorl.lenght-i+1)*sizeof(uint64_t));
					...
					memmove_temp(&gorl.list[i+2], &gorl.list[i], (gorl.lenght-i)*sizeof(uint64_t));
					gorl.list[i] = start;
...
//min-heap
inline void heapify(heapq_t *heap, size_t i, size_t size){
...
inline uint64_t get_min(heapq_t *heap, size_t *size){
...
void init_mm(){
	...
	mark_busy(boot_info->ptzone, total + (total&0xfff ? 4096 - (total&0xfff) : 0));
	mark_busy((void*)(((uint64_t)boot_info)&0xfff), 1<<16);

	// sht initialization
	//sht_ptr = allocate_pages(sht_size);
}